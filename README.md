...
- Web 管理页面：访问 `http://localhost:8080/admin`，支持 Ajax 操作反馈，减少不必要的整页刷新。
- 支持配置：
  - 项目 API Key。
  - 项目默认 SOCKS5 代理。
  - 多个 Gemini API Key。
...
- 请求日志：记录代理请求的时间、方法、路径、客户端 IP、状态码、使用的 Key、重试次数、耗时和错误信息，可在管理页面查看。
- 日志定时清理：可在管理页面配置日志保留天数，服务每小时自动清理过期日志，也支持手动清空。
...
| `LOG_PATH` | 与配置文件同目录的 `request_logs.jsonl` | 请求日志保存路径 |
...
7. 在“请求日志”中查看代理调用记录，并按需设置日志保留天数；设置为 `0` 表示不自动清理，最大 `365` 天。
8. 后续如果设置了项目 API Key，访问管理页面时会自动跳转到 `/login`，使用项目 API Key 登录后即可进入管理页面。
...
请求日志默认保存到配置文件同目录的 `request_logs.jsonl`，也可以通过 `LOG_PATH` 指定。日志内容不会记录请求头或请求体，并会对常见敏感查询参数（如 `key`）进行脱敏。
