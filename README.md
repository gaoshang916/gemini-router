...
4. 如某些 Gemini Key 需要独立代理，可在添加时填写代理，也可以在 Key 列表中单独修改。
5. 点击“测试”或“测试全部 Key”检查 Key 是否可用。
6. 后续如果设置了项目 API Key，访问管理页面时需要追加：

```text
http://localhost:8080/admin?admin_key=你的项目APIKey
```

也可以通过 `X-API-Key` 请求头访问管理接口。
